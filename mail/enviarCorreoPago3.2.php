...
SELECT
    ...
    MAX(pr.monto_pagado) AS monto_ultimo_pago,
...
$montoUltimoPago = number_format($result['monto_ultimo_pago'], 2);
...
try {
    ...
    $htmlTemplate = str_replace('[Cantidad Pagada]', $montoUltimoPago, $htmlTemplate);
    ...
    $mail->AltBody = "Hola $nombreCliente, confirmamos tu pago de $$montoUltimoPago. Categoría: $categoriaPago. Lote: $lote. Fecha: $ultimaFechaPago. Estado: $estatusPago. Restante: $$montoRestante.";
    ...
    if (isset($_FILES['archivo_pdf']) && $_FILES['archivo_pdf']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, ['pdf']) && $fileType === 'application/pdf') {
            if ($fileSize > (5 * 1024 * 1024)) {
                die('❌ Archivo PDF excede el tamaño permitido (5MB).');
            }
