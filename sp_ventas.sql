...
DROP PROCEDURE IF EXISTS registrar_venta_y_generar_calendario;
...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO ventas (
            ...
            p_precio_lista,
            p_descuento,
            p_autoriza_descuento,
            p_precio_venta,
            p_monto_enganche,
            p_fecha_pago_enganche,
            p_saldo_restante,
            p_forma_de_pago,
            p_plazo_meses,
            p_mensualidades,
        ...
        -- Obtenemos el ID recién insertado (por si lo necesitas para otra lógica futura)
        SET v_id_venta = LAST_INSERT_ID();
        SET dia_pago = DAY(p_fecha_inicio_pago);

        -- 3️⃣ Actualizar la propiedad a "Vendido"
        UPDATE propiedades
        SET disponibilidad = 'Vendido'
        WHERE id_lote = p_id_lote;

        -- 4️⃣ Generar el calendario de pagos

        -- 🟢 Registrar Enganche (si existe)
        IF p_monto_enganche > 0 THEN
            INSERT INTO calendario_pagos (
                id_lote,
                numero_pago,
        ...
        -- 🟢 Si es financiamiento (mensualidades)
        ELSEIF p_forma_de_pago = 'Financiamiento' THEN
            WHILE contador <= p_plazo_meses DO
                -- Calcular la fecha de vencimiento para el pago
                SET fecha_pago = DATE_ADD(p_fecha_inicio_pago, INTERVAL (contador - 1) MONTH);
                ...
                -- Insertar el registro de mensualidad en el calendario de pagos
                INSERT INTO calendario_pagos (
                    id_lote,
                    numero_pago,
                    fecha_vencimiento,
